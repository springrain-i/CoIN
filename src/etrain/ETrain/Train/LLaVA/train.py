...
def train():
    ...
    if training_args.local_rank in (-1, 0):
        train_runtime = train_result.metrics.get("train_runtime", float("nan"))
        steps_per_second = train_result.metrics.get("train_steps_per_second", float("nan"))
        print(
            f"[CoIN][TrainTiming] optimizer_steps={trainer.state.global_step}, "
            f"runtime_seconds={train_runtime:.3f}, steps_per_second={steps_per_second:.4f}",
            flush=True,
        )

    # This writes only trainer_state.json on rank 0.  The ZeRO-2 full
    # model/optimizer checkpoint is skipped separately in _save_checkpoint().
    trainer.save_state()
