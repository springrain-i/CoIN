...
class LLaVATrainer(Trainer):
            ...
            if self.sharded_ddp == ShardedDDPOption.SIMPLE:
                ...
                if self.args.local_rank in (-1, 0):
                    group_summaries = [
                        {
                            "lr": group["lr"],
                            "trainable_parameters": sum(param.numel() for param in group["params"]),
                        }
                        for group in self.optimizer.param_groups
                    ]
                    print(f"[CoIN][Optimizer] groups={group_summaries}", flush=True)
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if self.deepspeed is not None:
            get_zero_stage = getattr(self.deepspeed, "zero_optimization_stage", None)
            if callable(get_zero_stage):
                zero_stage = get_zero_stage()

        if getattr(self.args, "lora_enable", False) and zero_stage == 2:
            # CoIN task boundaries are persisted by save_trained_model() as a
            # lightweight adapter_model.bin + non_lora_trainables.bin pair.
            # The Trainer's epoch checkpoint asks DeepSpeed to materialize a
            # full training state on every ZeRO-2 rank and can exceed host RAM.
            if self.args.local_rank in (-1, 0):
                print(
                    "[CoIN][Checkpoint] ZeRO-2 LoRA: skipping full Trainer/DeepSpeed checkpoint",
                    flush=True,
                )
            return
    ...
    def save_trained_model(self, training_args):
        if training_args.lora_enable:
            ...
            is_writer = training_args.local_rank in (-1, 0)
            rank_zero_only = zero_stage == 2

            # ZeRO-2 keeps identical model parameters on every data-parallel
            # rank.  Extracting CPU clones on all eight ranks wastes host RAM
            # and can invoke the host OOM killer.  ZeRO-3 is intentionally not
            # handled this way: its partitioned parameters require every rank
            # to participate in GatheredParameters collectives below.
            if rank_zero_only and not is_writer:
            ...
            if is_writer:
                save_mode = (
                    "all ranks gather partitioned parameters; rank 0 writes files"
                    if zero_stage == 3
                    else "rank 0 extracts and writes files"
                )
                print(
                    f"[CoIN][Save] ZeRO stage={zero_stage}; {save_mode}",
                    flush=True,
                )
            ...
            if rank_zero_only and torch.distributed.is_available() and torch.distributed.is_initialized():
                torch.distributed.barrier()
        else:
