...
def forward_sdpa(
    ...
    # Prevent NaN from padding query positions propagating via residual.
    # Left-padded prefill produces all-inf attention rows for padding queries
    # → NaN attn_output. GPU matmul (IEEE 754) computes 0*NaN=NaN, so real
    # positions get contaminated in deeper layers. Zero out padding queries.
    if attention_mask is not None and not is_decode:
        q_pad = (attention_mask[:, :q_len] == 0)   # (B, q_len)
