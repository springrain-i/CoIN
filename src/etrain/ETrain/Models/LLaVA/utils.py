...
def create_LLaVA_model(training_args, model_args, data_args, bnb_model_from_pretrained_args, compute_dtype, local_rank):
    model_load_kwargs = dict(bnb_model_from_pretrained_args)
    if training_args.bits == 16:
        # Avoid constructing an fp32 copy before conversion to the requested
        # training dtype. Eight such copies exceed the host RAM on this node.
        model_load_kwargs.setdefault("torch_dtype", compute_dtype)

    with open(training_args.deepspeed, "r", encoding="utf-8") as ds_config_file:
        ds_zero_stage = json.load(ds_config_file).get("zero_optimization", {}).get("stage")
    serialize_model_load = ds_zero_stage == 2
    ...
    def load_base_model():
        if model_args.vision_tower is not None:
            if 'mpt' in model_args.model_name_or_path:
                ...
                return LlavaMPTForCausalLM.from_pretrained(
    ...
    if serialize_model_load:
        if local_rank is None or local_rank < 0 or not torch.distributed.is_initialized():
            raise RuntimeError("ZeRO-2 serialized model loading requires initialized distributed training")
        # ZeRO-3 rejects this flag because it supplies its own partition-aware
        # loading context. It is safe and useful only on the ZeRO-2 path here.
        ...
        for loading_rank in range(world_size):
            if local_rank == loading_rank:
                print(
                    f"[CoIN][ModelLoad] rank={local_rank}/{world_size}: loading bf16 base model",
                    file=sys.stderr,
                    flush=True,
                )
                model = load_base_model()
                model.to(device=torch.device("cuda", local_rank), dtype=compute_dtype)
                print(
                    f"[CoIN][ModelLoad] rank={local_rank}/{world_size}: moved base model to cuda:{local_rank}",
                    file=sys.stderr,
                    flush=True,
                )
