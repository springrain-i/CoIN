...
class _AttnStepRecord:
    __slots__ = ("step", "layer", "A_tt", "A_tv", "n_post_text")

    def __init__(self, step, layer, A_tt, A_tv, n_post_text):
        self.step        = step
        self.layer       = layer
        self.A_tt        = A_tt
        self.A_tv        = A_tv
...
class AttentionLogger:
    ...
    def _save_summary_csv(self, task_name: str, rank: int) -> None:
        if not self.records:
            return
        layer_data: dict = defaultdict(lambda: {"A_tt": [], "A_tv": []})
        for r in self.records:
