...
RESULT_DIR="${RESULT_ROOT}/ImageNet"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m ETrain.Eval.LLaVA.CoIN.model_vqa \
        ...
        --question-file "${INSTR_ROOT}/ImageNet/test.json" \
...
python -m ETrain.Eval.LLaVA.CoIN.eval_ImagetNet \
    --test-file "${INSTR_ROOT}/ImageNet/test.json" \
    --result-file "$output_file" \
    --output-dir "${RESULT_DIR}/${STAGE}"

echo "[eval][ImageNet] mode=${LORA_MODE} stage=${STAGE} -> ${RESULT_DIR}/${STAGE}"
