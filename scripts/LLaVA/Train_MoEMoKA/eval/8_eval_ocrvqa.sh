...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} "${PYTHON}" -m ETrain.Eval.LLaVA.CoIN.model_ocr_vqa \
        ...
        --question-file "${INSTR_ROOT}/OCRVQA/test.json" \
        --image-folder "$IMAGE_ROOT" \
        --answers-file "${RESULT_DIR}/${STAGE}/${CHUNKS}_${IDX}.jsonl" \
        --num-chunks "$CHUNKS" \
        --chunk-idx "$IDX" \
        --temperature 0 \
        --max-new-tokens "${MAX_NEW_TOKENS:-150}" \
        --merge-lora False \
        --lora-mode "$LORA_MODE" \
        --conv-mode vicuna_v1 &
done
...
"${PYTHON}" -m ETrain.Eval.LLaVA.CoIN.eval_ocrvqa \
    --annotation-file "${INSTR_ROOT}/OCRVQA/test.json" \
    --result-file "$output_file" \
    --output-dir "${RESULT_DIR}/${STAGE}"

echo "[eval][OCRVQA] mode=${LORA_MODE} stage=${STAGE} -> ${RESULT_DIR}/${STAGE}"
