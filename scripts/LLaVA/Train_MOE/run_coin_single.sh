...
resolve_single_model_path() {
  local task_name="$1"
  local candidate="${COIN_OUTPUT_ROOT}/${task_name}_llava_MOE_lora"
  if [[ -d "$candidate" ]]; then
    echo "$candidate"
    return
  fi
  ...
  if [[ "$task_name" == "VQAv2" ]]; then
    candidate="${COIN_OUTPUT_ROOT}/vqav2_llava_MOE_lora"
    if [[ -d "$candidate" ]]; then
      echo "$candidate"
      return
    fi
  ...
  echo "${COIN_OUTPUT_ROOT}/${task_name}_llava_MOE_lora"
}
