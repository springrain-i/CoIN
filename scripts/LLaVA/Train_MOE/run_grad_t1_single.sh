#!/bin/bash
# run_grad_t1_single.sh — run T1 ScienceQA gradient analysis on single GPU (no DeepSpeed)
#
# Usage:
#   bash scripts/LLaVA/Train_MOE/run_grad_t1_single.sh [task_num]
#   Default: task 1 (ScienceQA)
#
# Why no DeepSpeed: ZeRO-3 alloc/free cycles during model loading cause GPU memory
# fragmentation that prevents training from starting despite only ~4 GB being used.
# Single-GPU without ZeRO avoids this entirely; 7B LoRA training fits in 24 GB.
...
output_dir="${COIN_OUTPUT_ROOT}/${CKPT_NAMES[$TASK]}_single"
log_file="${LOG_DIR}/task${TASK}_${task_name}_single.log"

if [ "$TASK" -eq 1 ]; then
    prev_ckpt_arg=""
else
    prev_k=$((TASK - 1))
    prev_ckpt_arg="--previous_task_model_path ${COIN_OUTPUT_ROOT}/${CKPT_NAMES[$prev_k]}_single"
fi

echo "Task ${TASK}/8: ${task_name}  |  GPU: ${CUDA_VISIBLE_DEVICES}  |  No DeepSpeed"
