...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m ETrain.Eval.LLaVA.CoIN.model_vqa \
        ...
        --question-file /data4/wxl/MoBLoRA-backup/CoIN/playground/Instructions_Original/Grounding/test.json \
...
python -m ETrain.Eval.LLaVA.CoIN.eval_grounding \
    --test-file /data4/wxl/MoBLoRA-backup/CoIN/playground/Instructions_Original/Grounding/test.json \
    --result-file $output_file \
    --output-dir $RESULT_DIR/$STAGE \

python -m ETrain.Eval.LLaVA.CoIN.create_prompt \
    --rule ./ETrain/Eval/LLaVA/CoIN/rule.json \
    --questions /data4/wxl/MoBLoRA-backup/CoIN/playground/Instructions_Original/Grounding/test.json \
    --results $output_file \
