...
RESULT_DIR="./results/CoIN/LLaVA/Final_ON_ImageNet_MOE_only_vision"

for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m ETrain.Eval.LLaVA.CoIN.model_vqa \
        --model-path $MODELPATH \
        --model-base $BASE_MODEL_PATH \
        --question-file /data4/wxl/MoBLoRA-backup/CoIN/playground/Instructions_Original/ImageNet/test.json \
...
python -m ETrain.Eval.LLaVA.CoIN.eval_ImagetNet \
    --test-file /data4/wxl/MoBLoRA-backup/CoIN/playground/Instructions_Original/ImageNet/test.json \
